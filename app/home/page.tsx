...
import { Heart, Smile, Frown, Meh, Sun, Moon } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // Отлавливаем событие установки PWA
    const handler = (e: any) => {
      e.preventDefault();
      setDeferredPrompt(e);
  ...
  return (
    <div className="pb-24 pt-8 px-6 space-y-8 animate-in fade-in duration-500">
      ...
      {/* Кнопка установки (показывается только если доступна) */}
      {showInstallButton && (
        <button
  ...
  // При загрузке страницы достаем имя, которое сохранили при регистрации
  useEffect(() => {
    const savedName = localStorage.getItem('user_name');
    if (savedName) setUserName(savedName);
  }, []);
